...
<div id="main">
    ...
    <div id="site_content">
        ...
        <h1>Logowanie</h1>
        <form action="index.php?action=logging" method="post">
            <div class="form_settings2">
        <p><span><label>Username:</label></span>
            <input class="contact" type="text" name="username" value="" /></p>
        <p><span><label>Password:</label></span>
            <input class="contact" type="password" name="password" value="" /></p>
        <p style="padding-top: 15px"><span>&nbsp;</span><input class="submit" type="submit" name="contact_submitted" value="Zaloguj" /></p>
            </div>
        </form>
        <?php } else { ?>
            <h1>Jestes juz zalogowany</h1>
        <?php } ?>


    </div>
    <div id="footer">
        Copyright  <?php if($_SESSION['logged']==0){ ?><a href="index.php?action=logging">Zaloguj się</a><?php } else { ?><a href="index.php?action=adminPanel">Panel admina</a> <a href="index.php?action=logging&amp;option=logout"> Wyloguj</a> <?php } ?>
    </div>
</div>
</body>
